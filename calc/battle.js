...
function battle_calc_misc_attack(src, target, skill_id, skill_lv, mflag) {
    ...
    switch(skill_id) {
        ...
        case SKILL.SN_FALCONASSAULT: {
            ...
            damage = 80 + skill * 6 + 2 * Math.trunc(sstatus.dex / 15) + 2 * Math.trunc(sstatus.luk / 10);
        ...
        case SKILL.ASC_BREAKER: {
            md.damage_min = md.damage_max = 1.5 * skill_lv * sstatus.dex;
            md.crit_damage_min = md.damage_max;
            md.crit_damage_max = md.damage_max;
        s_ele = sstatus.rhw.ele;
        md.flag |= BF.WEAPON;
        break;
            ...
            default: {
                ...
                switch(skill_id) {
					...
					case SKILL.WZ_EARTHSPIKE:
						skillratio += 100; // x2 total (lv5 max = equivalent bolt lv10)
						break;
...
function battle_calc_element_damage(wd, src, target, skill_id, skill_lv) {
    ...
    if(sd && !skill_ignores_element(skill_id) && !skill_ignores_atkcard(skill_id) && ((wd.getAverageDamage() > 0 || wd.getAverageDamage2() > 0) || (wd.getAverageCritDamage() > 0 || wd.getAverageCritDamage2() > 0))) {
        if(SkillSearch(SKILL.ASC_EDP)) {
            let edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0);
            ...
            if(skill_id == SKILL.ASC_METEORASSAULT || skill_id == SKILL.ASC_BREAKER || skill_id == SKILL.AS_SPLASHER) {
                if(skill_id == SKILL.ASC_METEORASSAULT && (EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1))
                    edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using ice pick with meteor assault, edp rate is further halved to 5% of normal

                // If dual wielding, reduce EDP rate on right hand to compensate for left hand base damage
                    if(sd && sd.dual_wield)
                        edp_rate = Math.trunc(edp_rate * 100 / 100); // dual wield: EDP right hand reduced to 70% of normal
                    if(sd.weapontype1 == WEAPON.KATAR)
                        edp_rate = Math.trunc(edp_rate * 100/ 100); // Katar: EDP right hand reduced to 70% of normal

                if(skill_id == SKILL.ASC_BREAKER) {
                    if(EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1)
                        edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using ice pick with soul destroyer, edp rate is 16.5% of normal
                    else
                        edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using soul destroyer edp rate is 33% of normal

                    // If dual wielding, reduce EDP rate on right hand to compensate for left hand base damage
                    if(sd && sd.dual_wield)
                        edp_rate = Math.trunc(edp_rate * 60 / 100); // dual wield: EDP right hand reduced to 70% of normal
                    if(sd.weapontype1 == WEAPON.KATAR)
                        edp_rate = Math.trunc(edp_rate * 50/ 100); // Katar: EDP right hand reduced to 70% of normal
                }

                if(skill_id == SKILL.AS_SPLASHER) {
                    if(EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1)
                        edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0)    ; // if using ice pick with splasher, edp rate is 12.5% of normal
                    else
                        edp_rate = 20 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using other weapons with splasher, edp rate is 25% of normal
...
function battle_attr_fix(src, target, damage, atk_elem, def_type, def_lv, flag) {
    ...
    switch(atk_elem) {
        case ELE.FIRE:
            if(src.type == BL.PC) {
                if(sc_get(src, SC.VOLCANO)) // volcano +20% fire damage
        ...
        case ELE.WIND:
            if(src.type == BL.PC) {
                if(sc_get(src, SC.WHIRLWIND)) // whirlwind +20% wind damage
                    damage += Math.trunc((damage * sc_get(src, SC.WHIRLWIND).val3) / 100);
            }
            break;
        case ELE.WATER:
            if(src.type == BL.PC) {
                if(sc_get(src, SC.DELUGE)) // deluge +20% water damage
...
function battle_attack_sc_bonus(wd, src, target, skill_id, skill_lv) {
    ...
    if(skill_id == SKILL.AS_SONICBLOW_SL) {
        const edp_active = SkillSearch(SKILL.ASC_EDP) >= 1;
        const spirit_rate = edp_active ? 100 : 20;
        ATK_ADDRATE(wd, src, skill_id, c.A8_Skill14.value == 1 ? 25 : spirit_rate);
        CRIT_ATK_ADDRATE(wd, src, skill_id, c.A8_Skill14.value == 1 ? 25 : spirit_rate);
        }
...
function battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv) {
    ...
    switch(skill_id) {
        case SKILL.SM_BASH:
            skillratio += 30 * skill_lv;
            break;
            case SKILL.ITM_TOMAHAWK:  // ← ligne 3121
            skillratio += 0;
            break;
        case SKILL.BS_HAMMERFALL:
            skillratio += 50 * skill_lv;
            if(sd && sd.status.weapon == WEAPON.TWOHANDAXE)
                skillratio += 30* skill_lv;
            break;
		...
		case SKILL.MO_CHAINCOMBO:
			skillratio = 500; // 500% ATK fixe
			// +25% sur prochain Combo Finish
			if(sd) sc_start(src, SC.CHAINCOMBO_BONUS, 1, 0, 0, 0, 0, 0, 0, 0, 0, 10000);
			break;
		case SKILL.MO_COMBOFINISH:
			skillratio += 140 + 60 * skill_lv;
			// +25% si Chain Combo utilise juste avant
			if(sd && sc_get(src, SC.CHAINCOMBO_BONUS)) {
				skillratio += 25;
				sc_end(src, SC.CHAINCOMBO_BONUS);
			}
			break;
...
function skill_castfix(src, skill_id, skill_lv) {
    ...
    if(!flag) {
        ...
        // Assassin / Assassin Cross : STR + DEX = 210 pour no cast
        if(player.status.job_id == JOB.ASSASSIN || player.status.job_id == JOB.ASSASSIN_CROSS) {
            threshold = 250;
            scale = threshold - (player.battle_status.str + player.battle_status.dex);
        } else {
            threshold = 150;
            scale = threshold - player.battle_status.dex;
        }

        if(scale > 0)
            time = time * scale / threshold;
        else
            return 0;
    }
...
function skill_can_crit(skill_id) {
    switch(skill_id) {
        ...
        case SKILL.NJ_KIRIKAGE:
            return true;
        // Combo skills can crit if Critical Explosion is active
        case SKILL.MO_TRIPLEATTACK:
        case SKILL.MO_CHAINCOMBO:
        case SKILL.MO_COMBOFINISH:
        case SKILL.CH_TIGERFIST:
        case SKILL.CH_CHAINCRUSH:
            if(SkillSearch(SKILL.MO_EXPLOSIONSPIRITS) > 0 || TimeItemNumSearch(20))
                return true;
            return false;
    }
    return false;
