...
function updatePlayerDamageDisplay(d) {
    ...
    if(d.flag & BF.WEAPON) {
        ...
        if(d.skill_id == SKILL.CR_SHIELDBOOMERANG || d.skill_id == SKILL.CR_SHIELDBOOMERANG_SL || d.skill_id == SKILL.PA_SHIELDCHAIN) {
    ...
    let tripleAttackDmgMin = 0, tripleAttackDmgAvg = 0, tripleAttackDmgMax = 0, tripleAttackChance = 0;
    ...
    switch(d.skill_id) {
        case SKILL.NV_BASIC_ATTACK:
            ...
            if(SkillSearch(SKILL.MO_TRIPLEATTACK)) {
                ...
                tripleAttackChance = 30 - SkillSearch(SKILL.MO_TRIPLEATTACK);

                setVal("bSUB3", `${tripleAttackDamage.damage_min}~${tripleAttackDamage.damage_max} (${tripleAttackChance}%)`);
            }
        ...
        case SKILL.MO_COMBOFINISH:
            if(c.SkillSubNum.value == 1) {
                // +25% Chain Combo bonus
                minDamage = Math.floor(minDamage * 1.25);
                avgDamage = Math.floor(avgDamage * 1.25);
                maxDamage = Math.floor(maxDamage * 1.25);
                d.crit_damage_min = Math.floor(d.crit_damage_min * 1.25);
                d.crit_damage_max = Math.floor(d.crit_damage_max * 1.25);
            }
            damageMinText += minDamage;
            damageAvgText += avgDamage;
            damageMaxText += maxDamage;
        ...
        case SKILL.AM_ACIDTERROR: {
            const acidHits = 5;
            const acidPerHitMin = Math.floor(minDamage / acidHits);
            const acidPerHitAvg = Math.floor(avgDamage / acidHits);
            const acidPerHitMax = Math.floor(maxDamage / acidHits);
            damageMinText += (acidPerHitMin * acidHits) + ` (${acidPerHitMin} x ${acidHits} hits)`;
            damageAvgText += (acidPerHitAvg * acidHits) + ` (${acidPerHitAvg} x ${acidHits} hits)`;
            damageMaxText += (acidPerHitMax * acidHits) + ` (${acidPerHitMax} x ${acidHits} hits)`;
            break;
        }
    ...
    let canCrit = d.skill_id == 0 || skill_can_crit(d.skill_id);

    let critRatePct = (canCrit) ? (d.crit_rate / 10) : 0;
    ...
    if(d.skill_id == SKILL.NV_BASIC_ATTACK) {
        let taRate = hasTripleAttack ? tripleAttackChance / 100 : 0;
        ...
        let effectiveDA, effectiveCrit, effectiveDACrit, normalRate;

        if(player.status.job_id == JOB.SUPERNOVICE) {
            effectiveDACrit = (1 - taRate) * daRate * critRate;
            effectiveDA = (1 - taRate) * daRate * (1 - critRate);
            effectiveCrit = (1 - taRate) * (1 - daRate) * critRate;
            normalRate = (1 - taRate) * (1 - daRate) * (1 - critRate);
        } else {
            effectiveDACrit = 0;
            effectiveDA = (1 - taRate) * daRate;
            effectiveCrit = (1 - taRate) * (1 - daRate) * critRate;
            normalRate = 1 - effectiveTA - effectiveDA - effectiveCrit;
        }
        ...
        let daCritMin = d.crit_damage_min + (d.crit_damage2_min > 0 ? d.crit_damage2_min : 0);
        let daCritAvg = d.getAverageCritDamage() + ((d.crit_damage2_min > 0 || d.crit_damage2_max > 0) ? d.getAverageCritDamage2() : 0);
        let daCritMax = d.crit_damage_max + (d.crit_damage2_max > 0 ? d.crit_damage2_max : 0);

        // Crit display
        if(canCrit) {
            let totalEffectiveCrit = effectiveCrit + effectiveDACrit;
            ...
            setVal("CRInum", " (" + (totalEffectiveCrit * 100).toFixed(1) + "%)");

            if(player.status.job_id == JOB.SUPERNOVICE && effectiveDACrit > 0) {
                setVal("bSUB4name", "Critical Double Attack damage (chance)");
                setVal("bSUB4", daCritMin + "~" + daCritMax + " (" + (effectiveDACrit * 100).toFixed(1) + "%)");
            } else {
                setVal("bSUB4name", "");
                setVal("bSUB4", "");
            }
        ...
        minDamage = Math.floor(normalMin * normalRate + daMin * effectiveDA + tripleAttackDmgMin * effectiveTA + critMin * effectiveCrit + daCritMin * effectiveDACrit);
        avgDamage = Math.floor(normalAvg * normalRate + daAvg * effectiveDA + tripleAttackDmgAvg * effectiveTA + critAvg * effectiveCrit + daCritAvg * effectiveDACrit);
        maxDamage = Math.floor(normalMax * normalRate + daMax * effectiveDA + tripleAttackDmgMax * effectiveTA + critMax * effectiveCrit + daCritMax * effectiveDACrit);

        // Hit rate adjustment
        if(effectiveHitRate < 100) {
            avgDamage = Math.floor(avgDamage * (effectiveHitRate / 100));
        }
    } else {
        // Non-basic-attack skills: hit rate + crit adjustment
        if(effectiveHitRate < 100) {
            avgDamage = avgDamage * (effectiveHitRate / 100);
        }
...
function displayOtherInfo(val) {
    ...
    switch(val) {
        ...
        case 2: // heal amount display
            ...
            let healSelf = new Array(10).fill(0);
            let healOther = new Array(10).fill(0);
            ...
            displayData += `<tr><td><b>Heal Level 6</b></td><td class="center">${healSelf[5]}</td><td class="center">${healOther[5]}</td></tr>`;
            displayData += `<tr><td><b>Heal Level 7</b></td><td class="center">${healSelf[6]}</td><td class="center">${healOther[6]}</td></tr>`;
            displayData += `<tr><td><b>Heal Level 8</b></td><td class="center">${healSelf[7]}</td><td class="center">${healOther[7]}</td></tr>`;
            displayData += `<tr><td><b>Heal Level 9</b></td><td class="center">${healSelf[8]}</td><td class="center">${healOther[8]}</td></tr>`;
            displayData += `<tr><td><b>Heal Level 10</b></td><td class="center">${healSelf[9]}</td><td class="center">${healOther[9]}</td></tr>`;
        ...
        case 6: // increase hp/sp recovery
            ...
            if(n_A_JobClass() == JOB.ACOLYTE || n_A_JobClass() == JOB.MAGICIAN || player.status.job_id == JOB.SUPERNOVICE) {
                ...
                displayData += `<br>SP regen: ${3 * spRegenSelect + Math.floor(spRegenSelect * player.battle_status.max_sp / 500)} / 10 seconds`;
            }
            setVal("A_KakutyouData", displayData);
            break;
        ...
        case 10: // weight limit
            ...
            if(n_A_JobClass2() == JOB.STAR_GLADIATOR)
                maxWeight += maxWeight * c.A_KakutyouSelNum.value / 10; // knowledge of the SMS
        ...
        case 18: { // cast time/delay/aspd
            ...
            let totalReduceRate = gearReduction + bragiReduction;
            ...
            let totalDelay = Math.min(bragiDelay + gearDelay, 100);
    ...
    switch(val) {
        ...
        case 10: // weight limit
            ...
            if(hasStellarCourier) {
                selectData += `Knowledge of the SMS Lvl: <select name="A_KakutyouSelNum" onchange="StAllCalc()"></select><br>`;
            }
        ...
        case 13: // status resistance
            ...
            selectData += `<td>Rate to apply (%):</td><td><input type="tel" name="A_KakutyouStatusRate" value="0" size="4" onkeyup="StAllCalc()" style="text-align: center; width: 36px;"></td>`;
            selectData += `<td>Duration (secs):</td><td><input type="tel" name="A_KakutyouStatusDuration" value="0" size="4" onkeyup="StAllCalc()" style="text-align: center; width: 36px;"></td></tr>`;
            setVal("A_KakutyouSel", selectData);
