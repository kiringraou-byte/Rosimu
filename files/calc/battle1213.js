...
function battle_calc_misc_attack(src, target, skill_id, skill_lv, mflag) {
    ...
    switch(skill_id) {
        ...
        // ── MDEF avec ignore_mdef ────────────────────────────────
        let mdef = tstatus.mdef, mdef2 = tstatus.mdef2;
            if(sd) {
            ...
            let i = sd.indexed_bonus.ignore_mdef_by_race[tstatus.race] + sd.indexed_bonus.ignore_mdef_by_race[RC.ALL] +
            ...
            for(const it of status_get_race2(target)) i += sd.indexed_bonus.ignore_mdef_by_race2[it];
            if(status_has_mode(tstatus, MD.IGNOREIGNOREMDEF)) i = 0;
            mdef = Math.max(0, mdef - Math.trunc((mdef * cap_value(i, 0, 100)) / 100));
            }
         }

        // ── Dégâts ───────────────────────────────────────────────
        let int_dmg = ((3 * skill_lv * sstatus.int ) * (((100 - ( mdef / 2) ) / 100 )) - ( mdef2 / 2 ));
            md.damage_min = sstatus.matk_min + int_dmg;
            md.damage_max = sstatus.matk_max + int_dmg;
            md.crit_damage_min = md.damage_min;
            md.crit_damage_max = md.damage_max;
            s_ele = sstatus.rhw.ele;
            md.flag |= BF.MAGIC;
    ...
    if(sd && (skill_id == SKILL.ASC_BREAKER)) {
        md.damage_min += battle_calc_cardfix(BF.MAGIC, src, target, skill_id, s_ele, 0, md.damage_min, 2, md.flag);
        md.damage_max += battle_calc_cardfix(BF.MAGIC, src, target, skill_id, s_ele, 0, md.damage_min, 2, md.flag);
        md.crit_damage_min += battle_calc_cardfix(BF.MAGIC, src, target, skill_id, s_ele, 0, md.damage_min, 2, md.flag);
        md.crit_damage_max += battle_calc_cardfix(BF.MAGIC, src, target, skill_id, s_ele, 0, md.damage_min, 2, md.flag);
    }
...
function battle_calc_weapon_final_atk_modifiers(wd, src, target, skill_id, skill_lv) {
    ...
    if(skill_id == SKILL.ASC_BREAKER || skill_id == SKILL.AM_DEMONSTRATION) {
        ...
        wd.damage_min      += 500;
        wd.damage_max      += 500;
        wd.crit_damage_min += 500;
        wd.crit_damage_max += 500;
    }
        ...
        if(SkillSearch(SKILL.ASC_EDP)) {
            ...
            if(skill_id == SKILL.ASC_METEORASSAULT || skill_id == SKILL.ASC_BREAKER || skill_id == SKILL.AS_SPLASHER) {
                if(skill_id == SKILL.ASC_METEORASSAULT && (EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1))
                    edp_rate = 30 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using ice pick with meteor assault, edp rate is further halved to 5% of normal
                ...
                // If dual wielding, reduce EDP rate on right hand to compensate for left hand base damage
                    ...
                    if(sd.weapontype1 == WEAPON.KATAR)
                        edp_rate = Math.trunc(edp_rate * 60/ 100); // Katar: EDP right hand reduced to 70% of normal

                if(skill_id == SKILL.ASC_BREAKER) {
                    if(EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1)
                        edp_rate = 30 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using ice pick with soul destroyer, edp rate is 16.5% of normal
                    else
                        edp_rate = 30 * (SkillSearch(SKILL.ASC_EDP) + 0); // if using soul destroyer edp rate is 33% of normal
                ...
                if(skill_id == SKILL.AS_SPLASHER) {
                    if(EquipNumSearch(388) > 0 || EquipNumSearch(607) > 0 || c.A8_Skill14 == 1)
                        edp_rate = 30 * (SkillSearch(SKILL.ASC_EDP) + 0)    ; // if using ice pick with splasher, edp rate is 12.5% of normal
                    else
