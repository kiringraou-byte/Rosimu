...
function status_calc_max_hp(sd, vit) {
    ...
    if(sd.status.rebirth) {
        dmax *= 1.25;
    } else if(SkillSearch(SKILL.TK_MISSION_RANKER)) {
        dmax *= 3;
    }
...
function status_calc_str(player, str) {
    ...
    if(sc_get(player, SC.BLESSING))
        str += sc_get(player, SC.BLESSING).val1;
    if(SkillSearch(SKILL.SN_SIGHT))
        str += 5;
    if(SkillSearch(SKILL.TK_RUN_STR) && player.weapontype1 == WEAPON.FIST)
        str += 10;
    if(SkillSearch(SKILL.MC_LOUD) || sc_get(player, SC.LOUDEXCLAMATION)) // loud exclamation / crazy uproar
        str += 4;
...
function status_calc_agi(player, agi) {
    ...
    if(SkillSearch(SKILL.GS_INCREASING))
        agi += 4;
    if(SkillSearch(SKILL.AC_CONCENTRATION) || TimeItemNumSearch(4) || sc_get(player, SC.GLOOM_BOX)) { // improve concentration
...
function status_calc_int(player, int) {
    ...
    if(sc_get(player, SC.BLESSING)) { // blessing
        int += sc_get(player, SC.BLESSING).val1;
    }
    if(SkillSearch(SKILL.SN_SIGHT)) {
        int += 5;
    }
...
function status_calc_dex(player, dex) {
    ...
    if(sc_get(player, SC.BLESSING)) { // blessing
        dex += sc_get(player, SC.BLESSING).val1;
    }
    if(SkillSearch(SKILL.SN_SIGHT)) {
        dex += 5;
    }
...
function status_calc_batk(player, batk) {
    ...
    if(SkillSearch(SKILL.GS_GATLINGFEVER) && player.weapontype1 == WEAPON.GATLING)
        batk += 20 + 10 * SkillSearch(SKILL.GS_GATLINGFEVER);
    // Volcano / Deluge / Whirlwind : +6 ATK per level
    if(sc_get(player, SC.VOLCANO))
        batk += 6 * sc_get(player, SC.VOLCANO).val1;
    if(sc_get(player, SC.DELUGE))
        batk += 6 * sc_get(player, SC.DELUGE).val1;
    if(sc_get(player, SC.WHIRLWIND))
        batk += 6 * sc_get(player, SC.WHIRLWIND).val1;
    return batk;
}
...
function status_calc_flee(player, flee) {
    ...
    // Volcano / Deluge / Whirlwind : +4 Flee per level
    if(sc_get(player, SC.VOLCANO))
        flee += 4 * sc_get(player, SC.VOLCANO).val1;
    if(sc_get(player, SC.DELUGE))
        flee += 4 * sc_get(player, SC.DELUGE).val1;
    if(sc_get(player, SC.WHIRLWIND))
        flee += 4 * sc_get(player, SC.WHIRLWIND).val1;
    if(SkillSearch(SKILL.SG_MOON_COMFORT))
        flee += Math.trunc((player.status.base_level + player.battle_status.dex + player.battle_status.luk) / 10);
    if(SkillSearch(SKILL.RG_CLOSECONFINE))
        flee += 10;
    if(SkillSearch(SKILL.GS_ADJUSTMENT))
        flee += 30;
    if(SkillSearch(SKILL.GS_GATLINGFEVER))
        flee -= 5 * SkillSearch(SKILL.GS_GATLINGFEVER);
    if(SkillSearch(SKILL.LK_BERSERK))
...
function status_calc_def2(player, def2) {
    ...
    if(SkillSearch(SKILL.SG_SUN_COMFORT))
        def2 += Math.trunc((player.status.base_level + player.battle_status.dex + player.battle_status.luk) / 2);
...
function status_calc_mdef(player, mdef) {
    ...
    if(SkillSearch(SKILL.SM_ENDURE) || SkillSearch(SKILL.LK_CONCENTRATION) || CardNumSearch(95)) { // endure
        bonus = 0; // represents skill level
...
function status_calc_critical(player, critical) {
    ...
    if(SkillSearch(SKILL.MO_EXPLOSIONSPIRITS) || TimeItemNumSearch(20)) {
        ...
        if(SkillSearch(SKILL.MO_EXPLOSIONSPIRITS))
            ...
            bonus = 1;
        critical += 60 * bonus; // +6 Crit per level
    }
...
function status_calc_pseudobuff_matk(player, matk) {
    ...
    // Volcano / Deluge / Whirlwind : +6 MATK per level
    if(sc_get(player, SC.VOLCANO))
        matk += 6 * sc_get(player, SC.VOLCANO).val1;
    if(sc_get(player, SC.DELUGE))
        matk += 6 * sc_get(player, SC.DELUGE).val1;
    if(sc_get(player, SC.WHIRLWIND))
        matk += 6 * sc_get(player, SC.WHIRLWIND).val1;
 ...
 * 
 * @param {PlayerData} player 
 * @param {number} aspd_rate 
 * @returns 
 */
function status_calc_aspd_rate(player, aspd_rate) {
    let bonus;
    let max = 0;
    if(SkillSearch(SKILL.SG_STAR_COMFORT))
        max = (player.status.base_level + player.battle_status.dex + player.battle_status.luk);
    ...
    if((!sc_get(player, SC.QUAGMIRE) && !sc_get(player, SC.DECREASEAGI)) && (SkillSearch(SKILL.BS_ADRENALINE2) || sc_get(player, SC.ADRENALINE2))) {
        bonus = 0;
        if(SkillSearch(SKILL.BS_ADRENALINE2))
            bonus = 300;
    ...
    if((!sc_get(player, SC.QUAGMIRE) && !sc_get(player, SC.DECREASEAGI)) && (SkillSearch(SKILL.BS_ADRENALINE) || sc_get(player, SC.ADRENALINE) || (TimeItemNumSearch(5) && n_A_JobClass2() == JOB.ALCHEMIST))) {
        bonus = 0;
        if(SkillSearch(SKILL.BS_ADRENALINE))
            bonus = 300;
    ...
    if(SkillSearch(SKILL.GS_GATLINGFEVER) && player.weapontype1 == WEAPON.GATLING)
        max = Math.max(max, 20 * SkillSearch(SKILL.GS_GATLINGFEVER));
    ...
    // Volcano / Deluge / Whirlwind : +2% ASPD per level
    if(sc_get(player, SC.VOLCANO))
        aspd_rate -= 20 * sc_get(player, SC.VOLCANO).val1;
    if(sc_get(player, SC.DELUGE))
        aspd_rate -= 20 * sc_get(player, SC.DELUGE).val1;
    if(sc_get(player, SC.WHIRLWIND))
        aspd_rate -= 20 * sc_get(player, SC.WHIRLWIND).val1;
 ...
 */
function status_get_lv(src) {
    switch(src.type) {
        case BL.PC:
            return src.status.base_level;
        case BL.MOB:
            return src.level;
        default:
            return 0;
...
function sc_start(bl, type, val1 = 0, val2 = 0, val3 = 0, val4 = 0, val5 = 0, val6 = 0, val7 = 0, val8 = 0, val9 = 0, val10 = 0) {
    ...
    switch(type) {
        ...
        case SC.DELUGE:
            ...
            break;
        case SC.CHAINCOMBO_BONUS:
            // +25% Combo Finish damage after Chain Combo
            break;
        case SC.MINDBREAKER:
            val2 = 20 * val1;
            val3 = 12 * val1;
